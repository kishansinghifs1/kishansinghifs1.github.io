...
This week, I wasn't able to code much due to my re-exam but still I laid the foundation for the CI/CD pipeline setup for LibreHealth EHR project. The whole setup for the pipeline is done for now since we are writing the complete pipeline config file for the first time and our project is already on the gitlab so it was easy for the initial setup. 
